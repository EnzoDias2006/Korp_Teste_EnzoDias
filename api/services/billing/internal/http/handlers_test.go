...
func TestHealthRoutes(t *testing.T) {
	...
	t.Run("liveness does not depend on the database", func(t *testing.T) {
		...
		NewRouter(pingStub{err: errors.New("database offline")}, logger, nil, nil, nil).ServeHTTP(
			response,
	...
	t.Run("readiness succeeds when the database responds", func(t *testing.T) {
		response := httptest.NewRecorder()
		NewRouter(pingStub{}, logger, nil, nil, nil).ServeHTTP(
			response,
	...
	t.Run("readiness returns the stable envelope when the database fails", func(t *testing.T) {
		response := httptest.NewRecorder()
		request := httptest.NewRequest(http.MethodGet, "/health/ready", nil)
		request.Header.Set("X-Request-ID", "request-123")
		NewRouter(pingStub{err: errors.New("database offline")}, logger, nil, nil, nil).ServeHTTP(response, request)
	...
	t.Run("router applies request ID before CORS preflight", func(t *testing.T) {
		...
		NewRouter(pingStub{}, logger, []string{"http://localhost:4200"}, nil, nil).ServeHTTP(response, request)
